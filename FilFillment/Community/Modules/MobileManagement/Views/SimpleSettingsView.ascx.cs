...
namespace DotNetNuke.Modules.MobileManagement
{
    public partial class SimpleSettingsView : ModuleView, ISimpleSettingsView
    {
        ...
        private void lnkSave_OnClick(object sender, EventArgs e)
        {
            ...
            var nameCount = new RedirectionController().GetRedirectionsByPortal(ModuleContext.PortalId).Where(r => r.Name.ToLower() == name.ToLower()).Count();
            if (nameCount < 1)
            {
                SaveRedirection();
        ...
        private void SaveRedirection()
        {
            ...
            redirection.SourceTabId = (cboSourcePage.Visible) ? int.Parse(cboSourcePage.SelectedValue) : ModuleContext.PortalSettings.HomeTabId;

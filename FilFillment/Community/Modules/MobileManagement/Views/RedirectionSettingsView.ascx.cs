...
namespace DotNetNuke.Modules.MobileManagement
{
    ...
    public partial class RedirectionSettingsView : ModuleView, IRedirectionSettingsView
    {
        ...
        private void BindRedirection(int redirectId)
        {
            ...
            if (redirectId != Null.NullInteger)
            {
                ...
                if (redirect.SourceTabId != -1)
                {
                    ...
                    cboSourcePage.Select(redirect.SourceTabId.ToString(), false);
                }
                else
                {
                ...
                switch (redirect.TargetType)
                {
                    case TargetType.Portal:
                        ...
                        break;
                    case TargetType.Tab:
                            cboTargetPage.Select(redirect.TargetValue.ToString(), false);
                        break;
                    case TargetType.Url:
        ...
        private void BindTabs()
        {
			...
			cboSourcePage.Items.Clear();
			cboTargetPage.Items.Clear();
	        foreach (var tabInfo in tabs)
	        {
		        cboSourcePage.Items.Add(new DnnComboBoxItem(tabInfo.IndentedTabName, tabInfo.TabID.ToString()));
				cboTargetPage.Items.Add(new DnnComboBoxItem(tabInfo.IndentedTabName, tabInfo.TabID.ToString()));
	        }
        }

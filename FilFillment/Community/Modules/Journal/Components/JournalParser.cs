...
namespace DotNetNuke.Modules.Journal.Components {
	public class JournalParser {
		...
		public string GetList(int currentIndex, int rows) {
            ...
            if (ProfileId > 0) {
                journalList = jc.GetJournalItemsByProfile(OwnerPortalId, ModuleId, CurrentUser.UserID, ProfileId, currentIndex, rows);
            } else if (SocialGroupId > 0) {
                journalList = jc.GetJournalItemsByGroup(OwnerPortalId, ModuleId, CurrentUser.UserID, SocialGroupId, currentIndex, rows);
            } else {
                journalList = jc.GetJournalItems(OwnerPortalId, ModuleId, CurrentUser.UserID, currentIndex, rows);
            }
